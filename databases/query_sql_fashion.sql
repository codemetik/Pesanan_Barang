...
SELECT X.id_master, Y.id_produk, Z.id_ukuran, gambar, Y.nama, Z.nama_ukuran, stok, 
CONCAT('RP. ',FORMAT(harga,0)) AS harga, CONCAT('Rp. ',FORMAT(harga * stok,0)) AS total FROM data_master X
INNER JOIN produk Y ON X.id_produk = Y.id_produk
INNER JOIN ukuran_helm Z ON X.id_ukuran = Z.id_ukuran ORDER BY id_master ASC